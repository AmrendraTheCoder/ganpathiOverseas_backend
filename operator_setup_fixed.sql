-- =====================================================
-- GANPATHI OVERSEAS - OPERATOR DATABASE SETUP (CORRECTED)
-- Run this SQL in Supabase SQL Editor
-- =====================================================

-- Check what we're working with first
SELECT 'Starting operator database setup...' as status;
...
DO $$ BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'job_sheets' AND column_name = 'machine_id') THEN
        ALTER TABLE job_sheets ADD COLUMN machine_id UUID REFERENCES machines(id);
    END IF;
...
('hhhhhhhh-hhhh-hhhh-hhhh-hhhhhhhhhhhh', 'JOB000001', 'Business Card Printing', '1000 premium business cards with spot UV finish', (SELECT id FROM parties LIMIT 1), 'in_progress', 2, 1000, '4+0', 'Art Card 300gsm', '90mm x 54mm', 3500, 5000, CURRENT_DATE + INTERVAL '5 days', '44444444-4444-4444-4444-444444444444', 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee', '22222222-2222-2222-2222-222222222222'),
('iiiiiiii-iiii-iiii-iiii-iiiiiiiiiiii', 'JOB000002', 'Brochure Printing', '500 tri-fold brochures for marketing campaign', (SELECT id FROM parties LIMIT 1), 'pending', 1, 500, '4+4', 'Art Paper 150gsm', 'A4 tri-fold', 8000, 12000, CURRENT_DATE + INTERVAL '7 days', '55555555-5555-5555-5555-555555555555', 'dddddddd-dddd-dddd-dddd-dddddddddddd', '22222222-2222-2222-2222-222222222222'),
('jjjjjjjj-jjjj-jjjj-jjjj-jjjjjjjjjjjj', 'JOB000003', 'Annual Report Printing', '200 copies of 50-page annual report', (SELECT id FROM parties LIMIT 1), 'in_progress', 3, 200, '4+4', 'Art Paper 120gsm', 'A4', 15000, 22000, CURRENT_DATE + INTERVAL '10 days', '44444444-4444-4444-4444-444444444444', 'dddddddd-dddd-dddd-dddd-dddddddddddd', '22222222-2222-2222-2222-222222222222')
ON CONFLICT (id) DO UPDATE SET
