-- =====================================================
-- GANPATHI OVERSEAS - COMPLETE DATABASE SETUP WITH DEMO DATA
...
-- Step 11: Create RLS policies (Allow all operations for now)
-- ---------------------------------------------------------
CREATE POLICY IF NOT EXISTS "chart_of_accounts_all" ON chart_of_accounts FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "financial_transactions_all" ON financial_transactions FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "profit_loss_reports_all" ON profit_loss_reports FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "balance_sheet_reports_all" ON balance_sheet_reports FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "cash_flow_reports_all" ON cash_flow_reports FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "tax_reports_all" ON tax_reports FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "custom_reports_all" ON custom_reports FOR ALL USING (true) WITH CHECK (true);

-- Step 12: Create transaction number generation function
-- ---------------------------------------------------
...
-- Step 13: Create trigger function for auto-generating transaction numbers
-- ---------------------------------------------------------------------
...
-- Step 15: Insert Chart of Accounts (46 accounts for printing business)
-- ------------------------------------------------------------------
...
-- Step 16: Insert Demo Financial Transactions
-- ------------------------------------------
DO $$
DECLARE
...
-- Step 17: Update existing job sheets with UV and baking values
-- ----------------------------------------------------------
DO $$
BEGIN
    UPDATE job_sheets 
...
-- Step 18: Insert sample reports for testing
-- -----------------------------------------
DO $$
DECLARE
    first_user_id UUID;
...
-- Step 19: Success message and verification
-- ---------------------------------------
SELECT 
...
SELECT 
    ...
    (SELECT COUNT(*) FROM chart_of_accounts) as chart_of_accounts_count,
    (SELECT COUNT(*) FROM financial_transactions) as financial_transactions_count,
    (SELECT COUNT(*) FROM profit_loss_reports) as profit_loss_reports_count,
    (SELECT COUNT(*) FROM balance_sheet_reports) as balance_sheet_reports_count; 