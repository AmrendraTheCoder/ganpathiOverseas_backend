-- Complete Final Migration for Ganpathi Overseas Enterprise System
-- This script accounts for your existing database structure including:
-- quotation_requests, user_audit_log, user_permissions tables
-- Run this script in Supabase SQL Editor

-- =====================================================
-- STEP 1: CREATE ENUMS (SAFE - CHECK IF EXISTS FIRST)
-- =====================================================
...
-- =====================================================
-- STEP 3: ADD UPDATED_AT COLUMNS TO EXISTING TABLES
-- =====================================================

-- Add updated_at to all main tables if missing
DO $$ BEGIN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'party_orders' AND column_name = 'updated_at') THEN
        ALTER TABLE party_orders ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'quotation_requests' AND column_name = 'updated_at') THEN
        ALTER TABLE quotation_requests ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;
...
DO $$ BEGIN
    ...
    -- Check if role column exists and has the right type
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'role' AND data_type != 'USER-DEFINED') THEN
        -- If role exists but wrong type, rename it and create new one
        ALTER TABLE users RENAME COLUMN role TO old_role;
        ALTER TABLE users ADD COLUMN role user_role DEFAULT 'operator';
        -- Migrate old role values if possible
        UPDATE users SET role = 
            CASE 
                WHEN old_role = 'admin' THEN 'admin'::user_role
                WHEN old_role = 'supervisor' THEN 'supervisor'::user_role  
                WHEN old_role = 'finance' THEN 'finance'::user_role
                ELSE 'operator'::user_role
            END;
    ELSIF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'role') THEN
        ALTER TABLE users ADD COLUMN role user_role DEFAULT 'operator';
    END IF;
...
-- =====================================================
-- STEP 7: EXTEND PARTY_TRANSACTIONS TABLE
-- =====================================================
...
-- Don't create audit_logs if user_audit_log already exists
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_logs') 
    AND NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_audit_log') THEN
...
DROP TRIGGER IF EXISTS update_party_orders_updated_at ON party_orders;
CREATE TRIGGER update_party_orders_updated_at 
    BEFORE UPDATE ON party_orders 
    FOR EACH ROW EXECUTE PROCEDURE update_updated_at_column();
...
-- Enable RLS on new tables only (existing tables already have RLS)
DO $$ BEGIN
...
-- =====================================================
-- STEP 13: CREATE HELPER FUNCTIONS (UPDATE EXISTING)
-- =====================================================
...
-- =====================================================
-- STEP 14: CREATE RLS POLICIES FOR NEW TABLES ONLY
-- =====================================================

-- Only create policies for new tables (existing tables already have working policies)
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'machines') THEN
...
-- =====================================================
-- STEP 15: CREATE ENHANCED DASHBOARD FUNCTION
-- =====================================================
    ...
    CASE role
        WHEN 'admin' THEN
            SELECT json_build_object(
                ...
                'total_machines', (SELECT COUNT(*) FROM machines WHERE is_active = true),
                'quotation_requests', (SELECT COUNT(*) FROM quotation_requests WHERE created_at >= CURRENT_DATE - INTERVAL '30 days')
            ) INTO stats;
        ...
        WHEN 'supervisor' THEN
            SELECT json_build_object(
                ...
                'quotation_requests', (SELECT COUNT(*) FROM quotation_requests WHERE created_at >= CURRENT_DATE - INTERVAL '7 days')
            ) INTO stats;
        ...
        WHEN 'operator' THEN
            SELECT json_build_object(
                ...
                'machine_jobs', (SELECT COUNT(*) FROM job_progress WHERE operator_id = user_id AND status IN ('pending', 'in_progress'))
            ) INTO stats;
    END CASE;
...
SELECT 
    'Enhanced Tables: ' || COUNT(*) as result
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_type = 'BASE TABLE'
AND table_name IN ('parties', 'users', 'job_sheets', 'party_transactions', 'party_orders', 'quotation_requests', 'machines', 'expenses', 'job_progress', 'system_settings');
...
WHERE table_schema = 'public' 
AND table_name IN ('parties', 'users', 'job_sheets', 'party_transactions', 'machines', 'expenses', 'job_progress', 'system_settings')
GROUP BY table_name
ORDER BY table_name; 