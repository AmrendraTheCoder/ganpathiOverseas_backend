-- =====================================================
-- GANPATHI OVERSEAS - CORRECTED DATABASE FIX
...
-- Step 7: Create RLS policies (CORRECTED - no IF NOT EXISTS)
-- --------------------------------------------------------
DO $$ 
BEGIN
    ...
    -- Balance Sheet Reports policies
    BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '⚠️ Some RLS policies could not be created: %', SQLERRM;
END $$;
...
-- --------------------
SELECT 
    '🎉 CORRECTED DATABASE SETUP COMPLETE! 🎉' as result,
    ...
    '✅ RLS policies fixed and working' as policies_status; 